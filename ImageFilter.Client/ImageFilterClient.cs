...
namespace Vostok.Sample.ImageFilter.Client
{
    public class ImageFilterClient
    {
        ...
        {
            return await MakeRequest($"ImageFilter/{id}/BlackWhite");
        }
        ...
        public async Task<string> ApplyContrastAsync(string id, int amount)
        {
            ...
            return await MakeRequest($"ImageFilter/{id}/Contrast/{amount}");
        }
        
        public async Task<string> ApplyInvertAsync(string id)
        {
            return await MakeRequest($"ImageFilter/{id}/Invert");
        }
        ...
        public async Task<string> ApplyBrightnessAsync(string id, int amount)
        {
            ...
            return await MakeRequest($"ImageFilter/{id}/Brightness/{amount}");
        }
        ...
        public async Task<string> ApplyPixelateAsync(string id, int size)
        {
            ...
            return await MakeRequest($"ImageFilter/{id}/Pixelate/{size}");
        }
        
        public async Task<string> ApplyOilPaintAsync(string id)
        {
            return await MakeRequest($"ImageFilter/{id}/OilPaint");
        }
        
        public async Task<string> ApplyVignetteAsync(string id)
        {
            return await MakeRequest($"ImageFilter/{id}/Vignette");
        }
        
        public async Task<string> ApplyGlowAsync(string id)
        {
            return await MakeRequest($"ImageFilter/{id}/Glow");
        }
        ...
        public async Task<string> ApplyResizeAsync(string id, int width, int height)
        {
            ...
            return await MakeRequest($"ImageFilter/{id}/Resize/{width}x{height}");
        }

        private async Task<string> MakeRequest(string uri)
        {
            var result = await cluster.SendAsync(Request.Post(uri)).ConfigureAwait(false);
